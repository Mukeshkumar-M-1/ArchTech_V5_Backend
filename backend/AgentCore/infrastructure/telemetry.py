...
class TelemetryCollector:
    def __init__(self, cost_tracker: CostTracker):
        ...
        log.info("[TelemetryCollector] Initialized.")
    ...
    def record_llm_call(self, turn_id: int, prompt_tokens: int, completion_tokens: int, latency_ms: float):
        ...
        log.info(f"[TelemetryCollector] Recorded LLM Call (Turn {turn_id}): {prompt_tokens} in / {completion_tokens} out. Cost: ${turn_cost:.4f}")
    ...
    def record_tool_execution(self, turn_id: int, duration_ms: float):
        ...
        log.debug(f"[TelemetryCollector] Recorded Tool Execution (Turn {turn_id}): {duration_ms:.2f}ms")
