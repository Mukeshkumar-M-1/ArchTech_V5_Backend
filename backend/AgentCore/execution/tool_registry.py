...
class ToolRegistry:
    ...
    def __init__(self):
        ...
        log.info("[ToolRegistry] Initialized (empty)")
    ...
    def get_all(self) -> list[ToolDefinition]:
        ...
        log.info(f"[ToolRegistry] get_all: {len(names)} tools: {names}")
    ...
    def build_tool_definitions(self, context: dict | None = None) -> list[dict]:
        ...
        log.info(f"[ToolRegistry] build_tool_definitions: {len(definitions)} tools")
        return definitions
    
    def build_selected_tool_definition(self, tool_list: list) -> list[dict]: 
        """Helper to build all tool definitions for an API request from selected tools list.

        Args:
            tools_list: List of tools choosen for definition

        Returns:
            List of tool definition dicts suitable for LLM API.
        """
        tools_definitions = []
        for tool_item in self._tools.values():
            if (tool_item.name in tool_list):
                tools_definitions.append(
                    {
                        "type": "function",
                        "function": {
                            "name": tool_item.name,
                            "description": tool_item.get_description(),
                            "parameters": tool_item.input_schema,
                        },
                    }
                )
        log.info(f"[ToolRegistry] build_selected_tool_definition:  {len(tools_definitions)} tools")
    ...
    def register_builtins(self) -> None:
        ...
        for name in dir(builtin_module):
            ...
            if isinstance(obj, ToolDefinition):
                self.register(obj)
                count += 1
        log.info(f"[ToolRegistry] register_builtins: loaded {count} tools from builtins")
...
def get_selected_tool_definition(tools_list: list) -> list[dict]:
    """Helper to build all tool definitions for an API request from selected tools list.
    ...
    return registry.build_selected_tool_definition(tool_list=tools_list)