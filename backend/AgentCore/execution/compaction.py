...
class TokenEstimator:
    ...
    def estimate(messages: list[dict]) -> int:
        ...
        """
        encoder = TokenEstimator._get_encoder()
        total = 0
...
class AutoCompact:
    ...
    def run(
        ...
        """
        log.info("[AutoCompact] Checking %d messages against budget %d", len(messages), token_budget)
        now = time.time()

        if state.breaker_open_until > now:
            return messages, None

        tokens = TokenEstimator.estimate(messages)
        ...
        for i, msg in enumerate(messages):
            running += TokenEstimator.estimate([msg])
...
class CompactPipeline:
    ...
    def run(
        ...
        result, summary = AutoCompact.run(
            messages=result,
    ...
    def _apply_tool_compact(messages: list[dict]) -> list[dict]:
        ...
        for msg in messages:
            ...
            tool_path = Tool_compact.extract_path(tool_content)
            if tool_path:
                msg["content"] = (
                    f"Tool Name: {tool_name}\n"
                    f"[File: {tool_path}, "
    ...
    def reset(self) -> None:
        ...
        log.info("[CompactPipeline] Reset compaction state")
        self.state = CompactState()
