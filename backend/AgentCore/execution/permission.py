...
Auto-allowed tools: FileRead, FileWrite, FileEdit, Glob, Search, TaskList, TaskGet,
                    TaskOutput, Skill, SendMessage, TaskCreate, TaskUpdate, 
                    TaskStop, Agent, TodoWrite
...
if TYPE_CHECKING:
    from .executor import ToolDefinition
...
class PermissionChecker:
    """Tool permission checker with configurable rules."""

    # Tools that can execute without user confirmation
    AUTO_ALLOW = frozenset({
        "FileRead", "FileWrite", "FileEdit", "Glob", "Search",
        "TaskCreate", "TaskUpdate", "TaskStop", "Agent", "TodoWrite",
        "TaskList", "TaskGet", "TaskOutput", "Skill", "SendMessage", "Bash"
    })
    ...
    def __init__(self, mode: PermissionMode = PermissionMode.REQUIRE_PROMPT) -> None:
        ...
        """
        log.info("[PermissionChecker] Initialized with mode=%s", mode.value)
        self._mode = mode
    ...
    def mode(self, value: PermissionMode) -> None:
        ...
        """
        log.info("[PermissionChecker] Mode changed: %s -> %s", self._mode.value, value.value)
        self._mode = value
    ...
    def can_use(self, tool_name: str) -> bool:
        ...
        """
        log.info("[PermissionChecker] can_use: tool=%s, mode=%s", tool_name, self._mode.value)
        if self._mode == PermissionMode.FULL_AUTO:
            ...
            log.warning(f"[PermissionChecker] Unknown tool '{tool_name}' — denying by default")
            return False
        return False
    ...
    def get_denied_tools(self) -> list[str]:
        ...
        """
        log.info("[PermissionChecker] get_denied_tools: mode=%s", self._mode.value)
    ...
    def get_required_prompt_for(self, tool_name: str) -> Optional[str]:
        ...
        """
        log.info("[PermissionChecker] get_required_prompt_for: tool=%s", tool_name)
