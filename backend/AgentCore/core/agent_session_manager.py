"""
AgentSessionManager - Manages persistence, pause/resume, and crash recovery.
...
class AgentSessionManager:
    ...
    def __init__(self, session_dir: Path):
        ...
        log.info(f"[AgentSessionManager] Initialized with directory: {self.session_dir}")
    ...
    def save_session(self, session_id: str, agent_state_store: AgentStateStore) -> None:
        ...
        try:
            ...
            log.info(f"[AgentSessionManager] Saved session [{session_id}] to [{session_file}]")
        except Exception as exception:
            log.error(f"[AgentSessionManager] Failed to save session '{session_id}': {exception}", exc_info=True)
    ...
    def load_session(self, session_id: str) -> Optional[AgentStateStore]:
        ...
        if not session_file.is_file():
            log.info(f"[AgentSessionManager] Session file not found: {session_file}")
            return None
        ...
        try:
            ...
            log.info(f"[AgentSessionManager] Successfully loaded session '{session_id}'")
            return agent_state_store
        except Exception as exception:
            log.error(f"[AgentSessionManager] Failed to load session '{session_id}': {exception}", exc_info=True)
            return None
