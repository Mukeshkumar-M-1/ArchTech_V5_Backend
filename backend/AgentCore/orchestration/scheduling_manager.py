...
from AgentCore.orchestration.lease_manager import TaskQueue, LeaseManager, Dispatcher, AssignmentTracker
...
class SchedulerManager:
    ...
    def __init__(
        ...
        message_manager: SSEGenerationMessageManager
    ):
    ...
    async def run_scheduling_cycle(self, task_queue: TaskQueue) -> list:
        ...
        while task_queue.size() > 0:
            ...
            if not eligible_worker_agents:
                log.warning(f"[SchedulerManager] No eligible workers for Task {agent_task.task_id}. Re-evaluating later.")
                break
            ...
            if not new_agent_worker:
                log.info(f"[SchedulerManager] No workers available (all busy) for Task {agent_task.task_id}.")
            ...
            log.info(f"[SchedulerManager] Scheduled Task {assign_agent_task.task_id} -> \n Worker : [{new_agent_worker.agent_id}] (Score: [{new_agent_score:.1f}])")
