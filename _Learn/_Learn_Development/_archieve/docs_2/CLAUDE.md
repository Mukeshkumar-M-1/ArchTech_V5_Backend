...
This is a **reverse-engineered / decompiled** version of Anthropic's official Claude Code CLI tool. The goal is to restore core functionality while trimming secondary capabilities. Many modules are stubbed or feature-flagged off. TypeScript strict mode is enforced — **`bun run precheck` Must pass with zero errors** (Include typecheck + lint fix + test）

## Git Commit Message Convention

use **Conventional Commits** Specifications:
...
# Lint & Format (Biome) — Use precheck instead of calling it separately in daily development 
bun run lint # lint check (whole project) 
bun run lint:fix # auto-fix lint issues 
bun run format # format all (all projects) 
bun run check # lint + format check (whole project) 
...
# Full check (typecheck + lint fix + test) — Must be run after the task is completed
bun run precheck
...
Detailed test specifications, coverage status and improvement plans are available in `docs/testing-spec.md`。
...
- **Build**: `build.ts` executes `Bun.build()` with `splitting: true`, entry `src/entrypoints/cli.tsx`, output `dist/cli.js` + chunk files. Build enables 19 features by default (see Feature Flag section below). After building, `import.meta.require` is automatically replaced with the Node.js compatible version (the product can run on both bun/node). `vendor/audio-capture/` and `src/utils/vendor/ripgrep/` will be copied to `dist/vendor/` when building. 
- **Build (Vite)**: `vite.config.ts` + `scripts/post-build.ts`, chunk output to `dist/chunks/`. post-build also copies vendor files to `dist/vendor/`. 
- **Vendor path resolution**: After building, the chunk file is located under `dist/` or `dist/chunks/`, and the vendor binary is under `dist/vendor/`. `src/utils/ripgrep.ts` and `packages/audio-capture-napi/src/index.ts` both locate the dist root directory through `lastIndexOf('dist')` in the `import.meta.url` path, and then splice the `vendor/` sub-path to ensure that the paths are consistent under different build product levels. 
...
- **Lint/Format**: Biome (`biome.json`). Covers `src/`, `scripts/`, `packages/` and all projects (including `packages/@ant/`). `bun run lint` / `bun run lint:fix` / `bun run format` / `bun run check` / `bun run check:fix`. 42 rules were turned off due to decompiled code, leaving only the `recommended` baseline. 
- **Pre-commit**: husky + lint-staged. Automatically execute `biome check --fix` (TS/JS) and `biome format --write` (JSON) on the temporary file when submitting. 
- **CI Lint**: `ci.yml` executes `bunx biome ci.` after dependency installation and before type checking. If lint or formatting does not meet the standards, CI will fail. 
- **Defines**: Centrally managed in `scripts/defines.ts`. Current version `2.1.888`. 
- **CI**: GitHub Actions — `ci.yml` (lint + build + test), `release-rcs.yml` (RCS releases), `update-contributors.yml` (automatically update contributors).
...
- **precheck must pass** — `bun run precheck` (typecheck + lint fix + test) must have zero errors, and any modifications must not introduce new type/lint/test errors. 
...
- **Biome Configuration** — 42 lint rules turned off due to decompiled code, leaving only the `recommended` baseline. Formatting covers the entire project (`src/`, `scripts/`, `packages/`, including `packages/@ant/`). `.tsx` files use 120 line width + mandatory semicolon; other files use 80 line width + semicolon on demand. JSON formatting is enabled. `.editorconfig` is aligned with the Biome configuration (2-space indentation). After modifying any code you should run `bun run precheck` to confirm that there are no type/lint/format/test issues, and the pre-commit hook will automatically intercept unqualified commits. 
- **tsc and Biome conflict handling** — When tsc requires declaration of attributes (used for assignment) but biome reports `noUnusedPrivateClassMembers` (only write, no reading), use `// biome-ignore lint/correctness/noUnusedPrivateClassMembers: <reason>` to suppress lint warnings and retain type declarations. `biome ci` must have zero warnings. 
- **`@ts-expect-error` Maintenance** — Only keep `@ts-expect-error` if the code below actually has a type error. If the type system has been updated causing the directive to become unused (TS2578), simply remove the annotation. False comparisons resulting from MACRO substitution (e.g. `'production' === 'development'`) still need to retain `@ts-expect-error`. 
