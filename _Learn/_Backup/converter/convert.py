...
TEMPLATE_CONFIGS: dict[str, dict] = {
    "srs": {
        "navy_table_section_prefixes": set(),
...
def post_process_odt(odt_file_path, sections=None, template_type="srs", document_metadata=None):
        ...
        for xml_file_path in [content_xml_path, styles_xml_path]:
            ...
            if target_styles_element is not None:
                ...
                custom_styles_to_inject = [
                    ...
                    # 6. TOC_PageBreak: Forces the TOC onto its own page
                ...
                for style_name, style_family, style_attrs, property_element_tag, parent_style in custom_styles_to_inject:
                    ...
                    if not style_exists:
                        ...
                        for style_key_attr, style_value_attr in style_attrs.items():
                            if style_key_attr in ['list-style-name', 'default-outline-level', 'class']:
                ...
                if master_styles is not None:
                    ...
                    if not content_page_exists:
                        ...
                        if base_mp is not None:
                            ...
                            for fname in ['DOC_DATE', 'DOC_VER_MAJOR', 'DOC_VER_MINOR']:
                            ...
                            # Line 1: Left = "Software Requirement Specification" | Right = DOC_DATE
                            ...
                            uf_doc_date.set(text_name_attr, 'DOC_DATE')
                            uf_doc_date.text = document_metadata.get('DOC_DATE', '')
                            ...
                            # total_pages = ET.Element('{' + text_uri + '}table-total-pages')
                            # total_val = ET.Element('{' + text_uri + '}value')
                            # total_val.text = '1'
                            # total_pages.append(total_val)
                            # span_total.append(total_pages)
            ...
            if is_content_xml:
                ...
                if body_element is not None:
                    ...
                    for table_index, table in enumerate(tables_in_body_element):
                        ...
                        # Navy blue tables for specific section filename prefixes
                        is_navy_table = (section_filename and any(
                            section_filename.startswith(prefix)
                            for prefix in cfg["navy_table_section_prefixes"]
                        ))
                    ...
                    if office_text is not None:
                        ...
                        # If document_metadata provided, switch all chapters (from chapter 1) to ContentPage master
                        if document_metadata and h1_headings:
                            ...
                            if automatic_styles_collection is not None:
                                # Create ContentPageStart paragraph style (parent=Heading_20_1, master-page-name=ContentPage)
                                ...
                                # Add paragraph-properties for page break (since automatic styles may not inherit properties reliably)
                                ...
                                # Apply ContentPageStart to all h1 headings (from chapter 1)
